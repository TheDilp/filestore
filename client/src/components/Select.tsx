...
export function Select({
  ...
  const { base, labelClasses, container } = classes({ variant });
        ...
        <FloatingPortal>
          <FloatingFocusManager context={context} modal={false}>
            <div
              ...
              className="border-primary rounded-xl border bg-white shadow outline-0"
              style={{
                ...floatingStyles,
                overflowY: "auto",
                minWidth: 100,
            ...
            >
              {options.map((item, i) => (
                <div
                  ...
                  role="option"
                  className={`border-t-secondary cursor-pointer overflow-hidden border-t p-2 outline-0 transition-colors first:rounded-t-xl first:border-t-0 last:rounded-b-xl ${i === activeIndex || item?.value === value ? "bg-info-highlight/20 text-info" : "text-black"}`}
